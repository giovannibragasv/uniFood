...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCartDisplay() {
    ...
    totalDisplay.textContent = `Total: R$${total.toFixed(
      2
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updateCartDisplay() {
    ...
    itemsCount = cart.reduce((sum, item) => sum + item.quantity, 0);
    if (cartTotalDisplay) {
      cartTotalDisplay.textContent = `Total: R$${total.toFixed(
        2
      )} / ${itemsCount} item${itemsCount > 1 ? "s" : ""}`;
    }
  }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    header.addEventListener("click", function () {
      accordions.forEach((otherAccordion) => {
        if (otherAccordion !== accordion) {
          ...
          const otherInputs = otherAccordion.querySelectorAll(
            "input[type='radio']"
          );
  ...
  function checkIfPaymentIsSelected() {
    ...
    if (
      isCartaoSelecionado ||
      trocoValorRadio.checked ||
      semTrocoRadio.checked
    ) {
  ...
  function showSuccessMessage(message) {
    ...
    successElement.innerHTML =
      message + '<span class="close-btn">&times;</span>';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function atualizarTotalPedido() {
    ...
    pedidoTotalDisplay.textContent = `Total: R$${total.toFixed(2)} / ${
      cart.length
    } itens`;
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const solicitarReembolsoBtn = document.getElementById(
    "solicitar-reembolso-btn"
  );
  ...
  function iniciarProgresso() {
    ...
    }, 5000); // 20 segundos para cada passo
  }
  ...
  solicitarReembolsoBtn.addEventListener("click", function () {
    if (currentStep === 1) {
      ...
      const confirmarReembolso = confirm(
        "Você deseja reembolso pelo app?"
    );
      ...
      const confirmarReembolso = confirm(
        "Você deseja o reembolso pelo app?"
      )
      
      if (confirmarCancelamento) {
        alert("Você já passou do prazo de cancelamento.");
        window.location.href = "TelaCardapio.html";
      }
      ...
      if (confirmarReembolso) {
        alert("Você já ultrapassou o prazo de pedido de reembolso.");
        window.location.href = "TelaCardapio.html"
         }
    }
  });
